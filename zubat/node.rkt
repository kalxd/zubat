...
;; 是否包含该样式类
(define/curry (node-class? classname el)
  (-> string? sxml:element? boolean?)
...
(module+ test
  (test-case "node-class?"
    (check-false (node-class? "main" el))
    (check-false (node-class? "div" el1))
    (check-true (node-class? "button" el1))
    (check-true (node-class? "input" el2))))
